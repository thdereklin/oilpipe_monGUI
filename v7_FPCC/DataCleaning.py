...
def DataCleaning(filenames, seninfo_signal, start_clean_signal, end_clean_signal):
    ...
    for f in filenames:
        ...
        # Extra Pressure Signal
        if 'diff_press_01' not in sensor_name:
            sensor_name.append('diff_press_01')
            sensor_name.append('diff_press_02')
            sensor_name.append('diff_press_03')
        ...
        if temp_matrix.dtype == dtype('float64'):
            ...
            if counts != 0:
                ...
                null_idx = unique(list(null_idx)) # remove null index repition
                
                if len(null_idx) > 0:
                    temp_matrix = delete(temp_matrix, null_idx, 0)
        ...
        # Setup pressure signal difference
        if data_matrix.shape[1] <= 8:
            cor_df['diff_press_01'] = cor_df.iloc[:,2] - cor_df.iloc[:,3]
            cor_df['diff_press_02'] = cor_df.iloc[:,3] - cor_df.iloc[:,5]
            cor_df['diff_press_03'] = cor_df.iloc[:,2] - cor_df.iloc[:,5]
