...
def ModelPrediction(test_filename, model_filename, start_pred_signal, prederr_signal): 	
    ...
    for filename in test_filename:
        ...
        if df.columns[output_sen+1] == 'XY8002FR':
            # Process prediction
            temp_data = zeros((df.shape[0],1))            
            temp_data[record_vec] = pred_result
            df['XY8002FR_pred'] = temp_data
            
            if 'MX8004FR_pred' in df.columns:
                check_vec = df['MX8004FR_pred']
                if sum(check_vec) != 0:
                    pass
            else:
                df['MX8004FR_pred'] = zeros((df.shape[0],1))
            
            # Process residual
            temp_data = zeros((df.shape[0],1))
            temp_data[record_vec] = residual
            df['XY8002FR_resi'] = temp_data      
                        
            if 'MX8004FR_resi' in df.columns:
                check_vec = df['MX8004FR_resi']
                if sum(check_vec) != 0:
                    pass
            else:
                df['MX8004FR_resi'] = zeros((df.shape[0],1))
            
        elif df.columns[output_sen+1] == 'MX8004FR':
            # Process prediction
            temp_data = zeros((df.shape[0],1))
            temp_data[record_vec] = pred_result
            df['MX8004FR_pred'] = temp_data
            
            if 'XY8002FR_pred' in df.columns:
                check_vec = df['XY8002FR_pred']
                if sum(check_vec) != 0:
                    pass
            else:
                df['XY8002FR_pred'] = zeros((df.shape[0],1))
            
            # Process residual
            temp_data = zeros((df.shape[0],1))
            temp_data[record_vec] = residual
            df['MX8004FR_resi'] = temp_data

            if 'XY8002FR_resi' in df.columns:
                check_vec = df['XY8002FR_resi']
                if sum(check_vec) != 0:
                    pass
            else:
                df['XY8002FR_resi'] = zeros((df.shape[0],1))
